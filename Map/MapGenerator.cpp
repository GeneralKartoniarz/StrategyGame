...
TopologyGraph MapGenerator::ExtractTopology(const std::vector<Tile>& map, int mapWidth, int mapHeight, const ClimateConfig& config)
{
    TopologyGraph graph;
    ...
    for (const auto& tile : map)
    {
        ...
        for (const auto& edge : tile.cellEdges)
        {
            ...
            if (vertexMap.find(hash1) == vertexMap.end())
            {
                ...
                float distToCenter = std::sqrt(std::pow(node.position.x - mapCenter.x, 2) + std::pow(node.position.y - mapCenter.y, 2));
                float falloff = std::pow(distToCenter / maxDist, 2.5f);
                float combinedElev = continentalNoise.GetNoise(node.position.x, node.position.y) + (elevNoise.GetNoise(node.position.x, node.position.y) * 0.4f);
                
                node.elevation = combinedElev - falloff;
