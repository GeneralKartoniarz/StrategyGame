...
{
    if (noise < config.waterThreshold) return Elevation::Water;
    if (noise < config.plainsThreshold) return Elevation::Plains;
    if (noise < config.hillsThreshold) return Elevation::Hills;
    return Elevation::Mountains;
}

Temperature ClimateEngine::GetTemperature(float noise, const ClimateConfig &config) const
{
    if (noise < config.coldThreshold) return Temperature::Cold;
    if (noise < config.temperateThreshold) return Temperature::Temperate;
    return Temperature::Hot;
}

Moisture ClimateEngine::GetMoisture(float noise, const ClimateConfig &config) const
{
    if (noise < config.dryThreshold) return Moisture::Dry;
    if (noise < config.normalThreshold) return Moisture::Normal;
    return Moisture::Wet;
}

BiomeType ClimateEngine::DetermineBiome(Elevation elev, Temperature temp, Moisture moist) const
{
    if (elev == Elevation::Water) return BiomeType::Ocean;
    if (elev == Elevation::Mountains) return BiomeType::MountainPeak;

    switch (temp)
    {
    case Temperature::Cold:
        if (moist == Moisture::Dry) return BiomeType::IceSheet;
        return BiomeType::Tundra;
    case Temperature::Temperate:
        if (moist == Moisture::Dry) return BiomeType::Plains;
        if (moist == Moisture::Normal) return BiomeType::Forest;
        return BiomeType::Taiga;
    case Temperature::Hot:
        if (moist == Moisture::Dry) return BiomeType::Desert;
        if (moist == Moisture::Normal) return BiomeType::Plains;
